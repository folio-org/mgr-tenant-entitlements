...
@Component
public class DesiredStateApplicationsFlowProvider {
   ...
   */
  public DesiredStateApplicationsFlowProvider(List<ApplicationFlowFactory> flowFactories) {
  ...
  public Flow createFlow(StageContext stageContext) {
    ...
    var revokeDescriptors = descriptorsByType.get(REVOKE);
    if (isNotEmpty(revokeDescriptors)) {
      var layers = getApplicationDescriptorsLayers(REVOKE, revokeDescriptors);
      var lfp = new LayerFlowProvider(ctx, revokeDescriptors);
      var applicationLayerFlows = mapItems(layers, lfp::prepareLayerStage);
    }
