...
class ApplicationDescriptorTreeLoader {
  ...
  public List<ApplicationDescriptor> load(Set<String> applicationIds, UUID tenantId, String token) {
    var descriptors = getApplicationDescriptors(applicationIds, token);
  ...
  private void loadInstalledAppDependencies(ApplicationDescriptor descriptor,
    ...
    var dependencyApps = getApplicationDescriptors(dependencyIds, token);
    for (ApplicationDescriptor dependencyDescriptor : dependencyApps) {
  ...
  private static Set<String> resolveDependenciesToEntitledApps(Map<String, Dependency> dependenciesToBeLoaded,
    ...
    for (EntitlementEntity entitlement : entitled) {
  ...
  private static void testEntitledAppMatchesWithDependency(EntitlementEntity entitlement, Dependency dependency,
    ...
    if (!versionMatches(appVersion, depVersion)) {
  ...
  private static boolean versionMatches(String version, String toBeMatchedWith) {
    boolean result = SemverUtils.satisfies(version, toBeMatchedWith);
  ...
  private static Map<String, Dependency> collectUnresolvedDependencies(ApplicationDescriptor descriptor,
    ...
    for (Dependency dependency : descriptor.getDependencies()) {
      ...
      if (app == null) {
        ...
        if (!versionMatches(appVersion, dependency.getVersion())) {
  ...
  private static void checkAllApplicationsFound(Set<String> applicationIds, List<ApplicationDescriptor> descriptors) {
    var notFoundIds = new HashSet<>(applicationIds);

    descriptors.forEach(descriptor -> notFoundIds.remove(descriptor.getId()));

    if (isNotEmpty(notFoundIds)) {
      throw new RequestValidationException(
        "Applications not found by the given ids", APPLICATION_IDS, join(", ", notFoundIds));
    }
  }

  private List<ApplicationDescriptor> getApplicationDescriptors(Set<String> applicationIds, String token) {
    var orderedIds = toStream(applicationIds).sorted().toList();
    var descriptors = applicationManagerService.getApplicationDescriptors(orderedIds, token);
    checkAllApplicationsFound(applicationIds, descriptors);

    return descriptors;
  }

  private List<EntitlementEntity> loadEntitlementsByDependencyNames(UUID tenantId,
    Set<String> names) {
