...
class FailedFlowFinalizerTest {
  ...
  void execute_entitle_positive() {
    when(flowRepository.updateStatusIfCurrentIn(
      eq(FLOW_ID), eq(FAILED), eq(NON_TERMINAL_STATUSES), any(ZonedDateTime.class))).thenReturn(1);
  ...
  void execute_positive_flowIsAlreadyInTerminalStatus() {
    when(flowRepository.updateStatusIfCurrentIn(
      eq(FLOW_ID), eq(FAILED), eq(NON_TERMINAL_STATUSES), any(ZonedDateTime.class))).thenReturn(0);
