...
| VALIDATION_INTERFACE_COLLECTOR_MODE    | scoped        |  false   | Defines how required / provided interfaces collected from application descriptors for validation. `scoped` means the interfaces collected for each application and its dependencies, `combined` means the interfaces collected from all applications altogether |
